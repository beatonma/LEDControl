...
public class ColorUtils {
			...
			if (set != null) {
					...
					if (iterator.hasNext()) {
						...
						if (container.isCustom()) {
							...
							return getColor(container.swatch, container.color + 1);
						}
					}
			}
			return defaultColor;
	...
	// Get a ColorContainer or create a new one with defaultColor if it doesn't already exist
	public static ColorContainer getColorContainerFromPreference(SharedPreferences sharedPreferences, String key, int defaultColor) {
		Set<String> set = sharedPreferences.getStringSet(key, null);

		if (set != null) {
			Iterator<String> iterator = set.iterator();
			if (iterator.hasNext()) {
				return new ColorContainer(iterator.next());
			}
		}

		set = new HashSet<>();
		ColorContainer defaultContainer = new ColorContainer(defaultColor);
		set.add(defaultContainer.toString());
	...
	public static ColorContainer getColorContainerFromPreference(SharedPreferences sharedPreferences, String key) {
		Set<String> set = sharedPreferences.getStringSet(key, null);
	...
	public static void writeColorContainerToPreference(SharedPreferences sp, String key, ColorContainer container) {
	...
	public static int updateColorFormat(SharedPreferences preferences, String key, int defaultColor, boolean lock) {
		ColorContainer container = getColorContainerFromPreference(preferences, key, defaultColor);
		container.setLocked(lock);

		Log.d("", "Updated color container: "+  container.toString());

		writeColorContainerToPreference(preferences, key, container);

		return container.getChosenColor();
	}
	...
	public static int interpolate(int startColor, int endColor, float progress) {
		...
		hsv[0] = hsvStart[0] + (hsvEnd[0] - (hsvStart[0]) * progress);
		hsv[1] = hsvStart[1] + (hsvEnd[1] - (hsvStart[1]) * progress);
		hsv[2] = hsvStart[2] + (hsvEnd[2] - (hsvStart[2]) * progress);
